...
public class SPARKDriveTeleop extends Command {
  ...
  protected void execute() {
    ...
    double one = Math.abs(1 * rawOne) > 0.15 ? -3750 * (1/0.95) * (Math.abs(rawOne) - 0.05) * Math.signum(rawOne): 0; // left forwards allegedly
    double five = Math.abs(1 * rawFive) > 0.15 ? -3750 * (1/0.95) * (Math.abs(rawFive) - 0.05) * Math.signum(rawFive): 0; // right forwards
